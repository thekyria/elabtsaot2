...
int io::importCalibrationValues(string filename_, CalibrationEditor* cal){
  ...
  if (validateSchema( ":/xml_schemas/calibration.xsd", filename_ )){
    cout << "The XML file is not built according to the calibration schema." << endl;
    return 1;
  }
  ...
  if (cal==NULL) return 3;
  cal->hardReset();
  CalibrationXMLHandler calibrationImporter(&xmlfile, cal);
  int ans = calibrationImporter.importfile();
  if (ans) {
    cout << "Failed to open xml file! error code: " << ans << endl;
    return 4;
  }

  ans = calibrationImporter.importXML();
  if (ans) {
    cout << "Failed to import xml file! error code: " << ans << endl;
    return 5;
  }
...
int io::exportCalibrationValues(std::string filename_, CalibrationEditor* cal){
  ...
  fprintf(f, "\t<Devicenumber> %zu </Devicenumber>\n", cal->emu()->getUSBDevicesCount());
  fprintf(f, "</Info>\n");
  fflush(f);

  // --------------- Output devices' info ---------------
  fprintf(f, "<Devices>\n");
  if (cal->emu()->getUSBDevicesCount()!= static_cast<size_t>(cal->_master_store.size())){
    cout<< "ERROR" << endl;
  ...
  for(size_t devid = 0; devid != cal->emu()->getUSBDevicesCount(); ++devid){
    ...
    for(size_t node=0; node!=24; ++node){
      ...
      for (size_t ri = 0; ri!=static_cast<size_t>(REAL_IMAG.size()); ++ri){
        ...
        for (int rt = 0; rt!= (RESISTORS.size()-2); ++rt){ //Remove the ext
