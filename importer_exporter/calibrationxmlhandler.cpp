...
CalibrationXMLHandler::CalibrationXMLHandler(QFile* file, CalibrationEditor* cal):
    _cal(cal), _doc("XMLFile") { _xmlfile = file; }

CalibrationXMLHandler::~CalibrationXMLHandler() {}

int CalibrationXMLHandler::importfile(){
  bool ansBool = _xmlfile->open(QIODevice::ReadOnly);
  if(!ansBool) return 1;

  ansBool = _doc.setContent(_xmlfile);
  if(!ansBool) { _xmlfile->close(); return 2; }
...
int CalibrationXMLHandler::importXML(){
  ...
  while( !n2.isNull() ) { //Enumerate the devices
    ...
    for(size_t k=0; k!= static_cast<size_t>(_cal->_master_store.size()); ++k){
    ...
    for(size_t i=0; i!=static_cast<size_t>(TEST_NAMES.size()); ++i){
  ...
  for (size_t k=0; k!=static_cast<size_t>(imported.size()); ++k){
    if (!imported.at(k)){
