...
SchematicMover::SchematicMover( Powersystem const* pws,
                                PwsSchematicModel* smd,
                                QWidget* parent ) :
    SchematicViewer(pws, smd, parent),
    _pws(pws), _smd(smd){}

void SchematicMover::mousePressEvent(QMouseEvent* event){
  ...
  case SCHMOVE_STATE_IDLE:{
    ...
    if( _selected.indexOf(comp)>=0 ){ // Point is a selected component
      ...
      _selected.clear();
      _state = SCHMOVE_STATE_SELECT;
    } }
    break;

  case SCHMOVE_STATE_PREPAN:
    _state = SCHMOVE_STATE_PAN;
    break;

  case SCHMOVE_STATE_PREZOOM:
    _state = SCHMOVE_STATE_ZOOM;
    break;
...
void SchematicMover::mouseMoveEvent(QMouseEvent* event){
  ...
  case SCHMOVE_STATE_PAN:
    _xref += (_xmouse - _xclick);
    _yref += (_ymouse - _yclick);
    break;

  case SCHMOVE_STATE_MOVE:
    int k;
...
void SchematicMover::mouseReleaseEvent(QMouseEvent* event){
  ...
  case SCHMOVE_STATE_PAN:
    _state = SCHMOVE_STATE_PREPAN;
    break;

  case SCHMOVE_STATE_ZOOM:
    _state = SCHMOVE_STATE_PREZOOM;
    break;

  case SCHMOVE_STATE_SELECT:
    _selected = _getPointedComponents(QPointF(_xclick,_yclick),
                                     QPointF(_xmouse,_ymouse));
    _state = SCHMOVE_STATE_IDLE;
    break;
...
void SchematicMover::wheelEvent(QWheelEvent* event){
