...
GPFPQPipeline::GPFPQPipeline(size_t element_capacity, size_t ver_dim, size_t hor_dim) :
  Pipeline(element_capacity, ver_dim, hor_dim),
  Sset(_elementCountMax, 0),
  I0(_elementCountMax, 0) {}

void GPFPQPipeline::reset(){
  Pipeline::reset(); // Invoke reset of parent class
  // element_count_max, ver_id_max, hor_id_max remain unchanged
  Sset.resize(_elementCountMax, 0);
  I0.resize(_elementCountMax, 0);
}

int GPFPQPipeline::insert_element(size_t ver_pos, size_t hor_pos, Bus const& el, bool overwrite){
  // pipeline is full!
  if (_elementCount==_elementCountMax) return 1;
  // Vertical index out of bounds! Invalid insertion!
  if (ver_pos>=_verIdMax) return 2;
  // Horizontal index out of bounds! Invalid insertion!
  if (hor_pos>=_horIdMax) return 3;
  ...
  complex<double> U(polar(el.V,el.theta));
...
int GPFPQPipeline::remove_element(size_t ver_pos, size_t hor_pos){
  size_t k = 0;
  bool el_found = false;
