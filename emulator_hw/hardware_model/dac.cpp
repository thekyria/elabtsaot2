...
DAC::DAC(unsigned int tap, unsigned int resolution, double out_min, double out_max) :
...
int DAC::set_out(double out, double* p_mismatch, bool force){
  ...
  if (out<_out_min){
    if (force){
      set_tap(0);
      if (p_mismatch) *p_mismatch = _out - out;
      return 0;
    } else {
      return 1;
    }
  ...
  double out_max = _out_min + (_out_max-_out_min) * static_cast<double>(_tap_max) / static_cast<double>(_tap_max+1);
  if (out>out_max){
    if (force){
      set_tap(_tap_max);
      if (p_mismatch) *p_mismatch = _out - out;
  ...
  double tapd = (out - _out_min) / (_out_max - _out_min) * static_cast<double>(_tap_max+1) ;
...
int DAC::set_tap(unsigned int val){
  if (val>_tap_max) return 1;
  _tap = val;
  _out = _out_min + (_out_max - _out_min) * static_cast<double>(_tap) / static_cast<double>(_tap_max+1);
  return 0;
}
