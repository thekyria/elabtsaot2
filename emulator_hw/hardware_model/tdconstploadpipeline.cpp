...
TDConstPLoadPipeline::TDConstPLoadPipeline(size_t element_capacity,
                                       size_t ver_dim, size_t hor_dim) :
    Pipeline(element_capacity, ver_dim, hor_dim),
    Sconst(_elementCountMax, 0),
    I0(_elementCountMax, 0) {}

void TDConstPLoadPipeline::reset(){
  Pipeline::reset(); // Invoke reset of parent class
  // element_count_max, ver_id_max, hor_id_max remain unchanged
  Sconst.resize(_elementCountMax, 0);
  I0.resize(_elementCountMax, 0);
}

int TDConstPLoadPipeline::insert_element( size_t ver_pos, size_t hor_pos,
...
int TDConstPLoadPipeline::remove_element( size_t ver_pos, size_t hor_pos){
